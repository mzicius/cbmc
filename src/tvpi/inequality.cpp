...
std::vector<std::string> inequality::vars(std::shared_ptr<inequality> i1, std::shared_ptr<inequality> i2){
	auto v = vars();
	std::set<std::string> set(v.begin(),v.end());
	auto i1v = i1->vars();
    set.insert(i1v.begin(),i1v.end());
...
bool inequality::planar_entailed(std::shared_ptr<inequality> ineq1){
	...
	if (std::dynamic_pointer_cast<unary_inequality>(i)!=nullptr && std::dynamic_pointer_cast<unary_inequality>(i1)!=nullptr){

	     std::shared_ptr<unary_inequality> u  = std::dynamic_pointer_cast<unary_inequality>(i);
		 ...
		 std::shared_ptr<dyadic_inequality> d  = std::dynamic_pointer_cast<dyadic_inequality>(i);
...
bool inequality::planar_entailed(std::shared_ptr<inequality> ineq1,std::shared_ptr<inequality> ineq2){
	...
	if (std::dynamic_pointer_cast<unary_inequality>(i)!=nullptr && std::dynamic_pointer_cast<unary_inequality>(i1)!=nullptr&& std::dynamic_pointer_cast<unary_inequality>(i2)!=nullptr)
	{	
		 if(debugging)
	    ...
	    bool inconsistent = planar_entailed(true, -u1->a, 0, -u1->c, u2->a, 0, u2->c); 
	    ...
	    return inconsistent || planar_entailed(false, u->a, 0, u->c, u1->a, 0, u1->c, u2->a, 0, u2->c);
	}
	else
	{	 if(debugging)
	    ...
	    bool inconsistent = planar_entailed(true, -d1->a, -d1->b, -d1->c, d2->a, d2->b, d2->c);
	    //System.out.println("inconsistent: " + ineq1 + ", " + ineq2 + " [" + inconsistent + "]"); 
		//std::cout<<"Result of inconsistent: "<<inconsistent<<std::endl;
	    return inconsistent || planar_entailed(false, d->a, d->b, d->c, d1->a, d1->b, d1->c, d2->a, d2->b, d2->c);
	}


}

bool inequality::planar_entailed(bool strict, mp_integer a, mp_integer b, mp_integer c, mp_integer a1, mp_integer b1, mp_integer c1){
	...
	if(debugging)
	std::cout<<a<<" "<<b<<" "<<c<<" "<<a1<<" "<<b1<<" "<<c1<<std::endl;
	...
	if (sign(d) != 0){
	    ...
	    if (strict) return strictly_less(a*c1, a1*c);
	    return less_equal(a * c1, a1 * c);
    }
    else if (sign_a < 0){
	    if (strict) return strictly_greater(a*c1, a1*c);
	    return greater_equal(a*c1, a1*c);
    }
	else if (sign_b > 0){

		if (strict) return strictly_less(b*c1, b1*c);
	    return less_equal(b*c1, b1*c);
    }
    else if (sign_b < 0){
	    if (strict) return strictly_greater(b*c1, b1*c);
...
bool inequality::planar_entailed(bool strict,mp_integer a, mp_integer b, mp_integer c, mp_integer a1, mp_integer b1, mp_integer c1, mp_integer a2, mp_integer b2, mp_integer c2){
	...
	std::cout<<a<<" "<<b<<" "<<c<<" "<<a1<<" "<<b1<<" "<<c1<<" "<<a2<<" "<<b2<<" "<<c2<<std::endl;
	}
	...
	if (sign_d == 0)
	{
	    ...
	    return planar_entailed(strict, a, b, c, a1, b1, c1) || planar_entailed(strict, a, b, c, a2, b2, c2); 
	}
		...
		if(debugging)
		std::cout<<"d_lambda1: "<<d_lambda1<<" d_lambda2: "<<d_lambda2<<std::endl;
	    ...
	    if (sign_d > 0)
		    ...
		    if (strict) return strictly_less(c_prime, c*d);
			else return less_equal(c_prime, c*d);
	        }
	    }
	    else{
	        if (sign(d_lambda1) > 0 || sign(d_lambda2) > 0){
		    ...
		    if (strict) return strictly_greater(c_prime, c*d);
...
bool inequality::planar_unsatisfiable(std::shared_ptr<inequality> ineq1, std::shared_ptr<inequality> ineq2){
	...
	if (std::dynamic_pointer_cast<unary_inequality>(i)!=nullptr && std::dynamic_pointer_cast<unary_inequality>(i1)!=nullptr&& std::dynamic_pointer_cast<unary_inequality>(i2)!=nullptr){
	    ...
	    return planar_entailed(true, -u->a, 0, -u->c,
			           u1->a, 0, u1->c, u2->a, 0, u2->c);
	}
	    ...
	    return planar_entailed(true, -d->a, -d->b, -d->c, d1->a, d1->b, d1->c, d2->a, d2->b, d2->c);
	}
}
...
bool syntax_compare(std::shared_ptr<inequality> a, std::shared_ptr<inequality> b){
	
	return a->to_string().compare(b->to_string()) < 0 ;

}

bool lex_compare(std::shared_ptr<inequality> ineq1, std::shared_ptr<inequality> ineq2){

	if (ineq1->arity() == 2 && ineq2->arity() == 2){

		std::shared_ptr<dyadic_inequality> dyadic1 = std::dynamic_pointer_cast<dyadic_inequality> (ineq1);
		std::shared_ptr<dyadic_inequality> dyadic2 = std::dynamic_pointer_cast<dyadic_inequality> (ineq2);


	    if (dyadic1->x.compare(dyadic2->x) < 0) return true;
        else if (dyadic1->x.compare(dyadic2->x) > 0) return false;
	    else if (dyadic1->y.compare(dyadic2->y) < 0) return true;
        else if (dyadic1->y.compare(dyadic2->y) > 0) return false;
	    else return syntax_compare(ineq1, ineq2);
	}
	else if (ineq1->arity() == 1 && ineq2->arity() == 2){

		std::shared_ptr<unary_inequality> unary = std::dynamic_pointer_cast<unary_inequality> (ineq1);
	    std::shared_ptr<dyadic_inequality> dyadic = std::dynamic_pointer_cast<dyadic_inequality> (ineq2);
	  
	    if (unary->x.compare(dyadic->x) < 0) return true;
        else if (unary->x.compare(dyadic->x) > 0) return false;
	    else return syntax_compare(unary, dyadic);
	}
		...
		std::shared_ptr<unary_inequality> unary1 = std::dynamic_pointer_cast<unary_inequality> (ineq1);
		std::shared_ptr<unary_inequality> unary2 = std::dynamic_pointer_cast<unary_inequality> (ineq2);
	  
	    if (unary1->x.compare(unary2->x) < 0) return true;
        else if (unary1->x.compare(unary2->x) > 0) return false;
	    else return syntax_compare(unary1, unary2);
	}
	else{
...
void show(std::shared_ptr<inequality> i){
	...
	if (std::dynamic_pointer_cast<unary_inequality>(i)!=nullptr){
		...
		std::shared_ptr<dyadic_inequality> d = std::dynamic_pointer_cast<dyadic_inequality>(i);
		if(d->b>=0) sign ="+";
		std::cout<<d->a<<d->x<<sign<<d->b<<d->y<<"\u2264"<<d->c<<std::endl;
	}

	else if(std::dynamic_pointer_cast<constant_inequality>(i)!=nullptr){
		std::shared_ptr<constant_inequality> c = std::dynamic_pointer_cast<constant_inequality>(i);
...
void normalize_coeff(std::vector<std::shared_ptr<inequality>> &xs){
	...
	if (std::dynamic_pointer_cast<unary_inequality>(xs[i])!=nullptr){
		 ...
		 std::shared_ptr<unary_inequality> u  = std::dynamic_pointer_cast<unary_inequality>(xs[i]);
...
void print_direction(inequality::direction d){
        ...
        std::string label;
        if(d==inequality::East) label="East";
        else if (d==inequality::North) label="North";
        else if (d==inequality::West) label= "West";
        else if (d==inequality::South) label ="South";
...
void print_ordering(inequality::ordering o){
	std::string label;
	if(o==inequality::LT) label="LESS";
	else if(o==inequality::EQ) label="EQUAL";
	else if (o==inequality::GT) label="GREATER";
	std::cout<<label<<std::endl;
}

inequality::ordering cmp_angle(std::shared_ptr<inequality> a, std::shared_ptr<inequality> b){

inequality::ordering res = inequality::EQ;

	if (std::dynamic_pointer_cast<dyadic_inequality>(a)!=nullptr && std::dynamic_pointer_cast<dyadic_inequality>(b)!=nullptr){

	std::shared_ptr<dyadic_inequality> i1  = std::dynamic_pointer_cast<dyadic_inequality>(a);
	std::shared_ptr<dyadic_inequality> i2  = std::dynamic_pointer_cast<dyadic_inequality>(b);
	...
	inequality::direction dir_b = b->calc_direction();
		...
		if(dir_a==dir_b){
			...
			if(dir_a>dir_b) res = inequality::GT;
			else			res = inequality::LT;
...
mp_integer determinant(std::shared_ptr<inequality> a,std::shared_ptr<inequality> b){
...
bool less_than_pi(std::shared_ptr<inequality> a,std::shared_ptr<inequality> b){
	...
	int diff = abs(b->calc_direction() - a->calc_direction()+4) %4;
	  ...
	  if (diff==0 && det >=0) return true; 
	  else if (diff==2 && det> 0 ) return true; 
...
rationalt dist_to_ineq(vertex ver,std::shared_ptr<inequality> a){
	...
	if (std::dynamic_pointer_cast<dyadic_inequality>(a)!=nullptr ){
		...
		dis = ver.x * rationalt(i1->a) + ver.y * rationalt(i1->b)- rationalt(i1->c);
...
vertex calc_intersection(std::shared_ptr<inequality> a,std::shared_ptr<inequality> b){
...
rationalt calc_shift(std::shared_ptr<inequality> a,std::shared_ptr<inequality> b){
...
std::shared_ptr<inequality> add_shift(rationalt r,std::shared_ptr<inequality> a){
...
new_coefs.erase(std::remove_if(new_coefs.begin(),new_coefs.end(),[](mp_integer i){return i.is_zero();}),new_coefs.end());

d = gcd_vector(new_coefs);

}

return std::make_shared<dyadic_inequality>(x,y,new_a/d,new_b/d,new_c/d);
	

}

std::optional<rationalt>calc_dist(std::shared_ptr<inequality> a,std::shared_ptr<inequality> e,std::shared_ptr<inequality> b){


	if (std::dynamic_pointer_cast<dyadic_inequality>(a)!=nullptr&&std::dynamic_pointer_cast<dyadic_inequality>(e)!=nullptr&&std::dynamic_pointer_cast<dyadic_inequality>(b)!=nullptr){

	std::shared_ptr<dyadic_inequality> i1  = std::dynamic_pointer_cast<dyadic_inequality>(a);
	std::shared_ptr<dyadic_inequality> i2  = std::dynamic_pointer_cast<dyadic_inequality>(b);
	std::shared_ptr<dyadic_inequality> it  = std::dynamic_pointer_cast<dyadic_inequality>(e);
...
bool inbetween(std::shared_ptr<inequality> a,std::shared_ptr<inequality> e,std::shared_ptr<inequality> b){
	...
	if(cmp_angle(i1,i2)==inequality::EQ) result = true;
	else{
	
	bool r1 = less_than_pi(i1,it);
	bool r2 = less_than_pi(it,i2);

	if(r1&&r2) result = determinant(i1,it) >=0 && determinant(it,i2)>=0;
	else if(r1&&!r2) result = determinant(i2,i1)>=0;
	else if(!r1&&r2) result = determinant(i2,i1)>=0;
	else if(!r1&&!r2) result = equal_to_pi(i1,i2) && determinant(i1,it)>=0;
...
cs.erase(std::remove_if(cs.begin(),cs.end(),[](std::shared_ptr<inequality> a){

if (std::dynamic_pointer_cast<dyadic_inequality>(a)!=nullptr){

	std::shared_ptr<dyadic_inequality> i  = std::dynamic_pointer_cast<dyadic_inequality>(a);
...
void rm_syn_red (std::vector<std::shared_ptr<inequality>> &cs){
	
	cs.erase(std::unique(cs.begin(),cs.end()),cs.end());

}

std::vector<std::shared_ptr<inequality>> rm_local_red (std::vector<std::shared_ptr<inequality>> &cs){
	...
	std::cout<<"prev: "<<prev<<" cur: "<<cur<<" x: "<<x<<std::endl;
...
std::optional<std::shared_ptr<inequality>> delta_combination(rationalt fr1,std::shared_ptr<inequality> a, rationalt fr2, std::shared_ptr<inequality> b){

mp_integer check = (fr1.get_numerator() * fr2.get_numerator())/(fr1.get_denominator()*fr2.get_denominator());
...
std::vector<std::shared_ptr<inequality>> gen_delta_comb(std::optional<rationalt> mDInner,std::shared_ptr<inequality> oC,std::optional<rationalt> mDOuter,std::shared_ptr<inequality> iP, std::vector<std::shared_ptr<inequality>> other){
...
void show_details(std::shared_ptr<inequality> iP,std::shared_ptr<inequality>  iC,std::vector<std::shared_ptr<inequality>> is, std::shared_ptr<inequality> oP,std::shared_ptr<inequality> oC,std::vector<std::shared_ptr<inequality>> os,std::string label,std::optional<rationalt> d){
...
void advance_outer(std::vector<std::shared_ptr<inequality>> &res,std::shared_ptr<inequality> iP,std::vector<std::shared_ptr<inequality>> &is,std::shared_ptr<inequality> oP,std::vector<std::shared_ptr<inequality>> &os){
...
void advance_inner(std::vector<std::shared_ptr<inequality>> &res,std::shared_ptr<inequality> iP,std::vector<std::shared_ptr<inequality>> &is,std::shared_ptr<inequality> oP,std::vector<std::shared_ptr<inequality>> &os){
...
extern bool is_redundant(std::shared_ptr<inequality> a, std::shared_ptr<inequality> b,std::shared_ptr<inequality> c){
