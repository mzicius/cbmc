...
void complete::closure(std::string in_dir, std::string out_dir)
{
  ...
  con_label =
    full_label.substr(first_dash_pos + 1, second_dash_pos - first_dash_pos - 1);

  plot_inputs_results.open(
    "../../plots/" + dim_label + "/" + dim_label + "D-IR.txt",
    std::ios_base::app);
  ...
  for(auto v : vs)
  {
    ...
    {
      //heavy data
      detailed_stats << "number of result calls for " << in_result->to_string()
                     << " is: " << intermediate_result_call << std::endl;
    }
...
complete::closure(std::vector<std::shared_ptr<inequality>> cs)
{
  std::vector<std::shared_ptr<inequality>> vs;
  for(std::shared_ptr<inequality> c0 : cs)
  {
...
std::vector<std::shared_ptr<inequality>>
complete::insert(std::vector<std::shared_ptr<inequality>> cs)
{
...
std::vector<std::shared_ptr<inequality>> complete::insert(
...
std::vector<std::shared_ptr<inequality>> complete::result(
  std::vector<std::shared_ptr<inequality>> cs1,
  std::vector<std::shared_ptr<inequality>> cs2)
{
...
std::vector<std::shared_ptr<inequality>>
complete::result(std::shared_ptr<inequality> i1, std::shared_ptr<inequality> i2)
{
  ...
  if(sign(a1) * sign(a2) < 0 && x1 == x2)
  {
    ...
    result.push_back(
      inequality_factory::make_inequality(y1, y2, a2_abs * b1, a1_abs * b2, c));
  }
  else if(sign(a1) * sign(b2) < 0 && x1 == y2)
  {
    ...
    result.push_back(
      inequality_factory::make_inequality(y1, x2, b2_abs * b1, a1_abs * a2, c));
  }
  ...
  if(sign(b1) * sign(a2) < 0 && y1 == x2)
  {
    ...
    result.push_back(
      inequality_factory::make_inequality(x1, y2, a2_abs * a1, b1_abs * b2, c));
  }
  else if(sign(b1) * sign(b2) < 0 && y1 == y2)
  {
    ...
    result.push_back(
      inequality_factory::make_inequality(x1, x2, b2_abs * a1, b1_abs * a2, c));
  }
...
std::vector<std::shared_ptr<inequality>> complete::project(
  std::vector<std::shared_ptr<inequality>> cs,
  std::vector<std::string> vars)
{
  ...
  for(auto i : cs)
  {
    ...
    //print_vars(contents);
    //std::cout<<std::endl;
    if(std::includes(set.begin(), set.end(), contents.begin(), contents.end()))
      project.push_back(i);
  }
  return project;
}

std::vector<std::shared_ptr<inequality>> complete::remove_constant_inequalities(
  std::vector<std::shared_ptr<inequality>> cs)
{
  ...
  for(auto c : cs)
  {
    ...
    {
      std::shared_ptr<constant_inequality> ci =
        std::dynamic_pointer_cast<constant_inequality>(c);
...
std::vector<std::shared_ptr<inequality>>
complete::remove_redundant_unary_inequalities(
  std::vector<std::shared_ptr<inequality>> cs)
{
  ...
  for(size_t i = 0; i < us.size(); i++)
  {
    ...
    {
      if(i != j)
        entailed = entailed || us.at(i)->planar_entailed(us.at(j));
    }
    if(!entailed)
...
std::vector<std::shared_ptr<inequality>> complete::inc_complete(
  std::vector<std::shared_ptr<inequality>> cs,
  std::shared_ptr<inequality> c0)
{
  ...
  for(auto i : cs)
  {
    std::vector<std::shared_ptr<inequality>> rs =
      remove_constant_inequalities(result(c0, i));
    ...
    std::vector<std::shared_ptr<inequality>> ts =
      remove_constant_inequalities(result(rs, cs));
    ...
    for(std::string y : vars)
    {
      if(x.compare(y) <= 0)
      {
        ...
        for(auto i : project_as)
        {
          ...
          if(is_bottom(project_cs))
            return make_bottom();
        }
        remove_all(residual_cs, project_cs);

        residual_cs.insert(
          residual_cs.end(), project_cs.begin(), project_cs.end());
...
extern bool is_bottom(std::vector<std::shared_ptr<inequality>> vec)
{
  ...
  if(std::dynamic_pointer_cast<constant_inequality>(in) != nullptr)
  {
    std::shared_ptr<constant_inequality> c =
      std::dynamic_pointer_cast<constant_inequality>(in);
    return c->sat == false;
  }
  else
  {
