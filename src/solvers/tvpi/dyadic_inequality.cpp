...
dyadic_inequality::dyadic_inequality(
  std::string _x,
  std::string _y,
  mp_integer _a,
  mp_integer _b,
...
std::vector<coordinate> dyadic_inequality::intercepts(
  std::string x,
  std::string y,
  int x_min,
  int y_min,
  ...
  int y_max)
{
  ...
  if(sign_a() != 0)
  {
    ...
    if(x_min <= x_at_y_min && x_at_y_min <= x_max)
      intercepts.push_back(coordinate(x_at_y_min, y_min));
    if(x_min <= x_at_y_max && x_at_y_max <= x_max)
      intercepts.push_back(coordinate(x_at_y_max, y_max));
  }
  ...
  if(sign_b() != 0)
  {
    ...
    if(y_min <= y_at_x_min && y_at_x_min <= y_max)
      intercepts.push_back(coordinate(x_min, y_at_x_min));
    if(y_min <= y_at_x_max && y_at_x_max <= y_max)
      intercepts.push_back(coordinate(x_max, y_at_x_max));
  }

  return intercepts;
}

std::vector<coordinate> dyadic_inequality::halfspace(
  std::string x,
  std::string y,
  int x_min,
  int y_min,
  ...
  int y_max)
{
  std::vector<coordinate> intercept =
    intercepts(x, y, x_min, y_min, x_max, y_max);
  ...
  if(sign(a) == -1)
    x_stroke_width = -x_stroke_width;
  if(sign(b) == -1)
    y_stroke_width = -y_stroke_width;
...
dyadic_inequality::widen(std::vector<std::string> vars)
{
  if(vars.size() > 2)
  {
    return nullptr;
...
std::string dyadic_inequality::to_string()
{
  ...
  if(a_abs_one && a_sign < 0)
    sb.append("-" + x);
  else if(a_abs_one && a_sign > 0)
    sb.append(x);
  else if(a_sign < 0)
    sb.append(integer2string(a) + x);
  else if(a_sign > 0)
    sb.append(integer2string(a) + x);

  if(b_abs_one && b_sign < 0)
    sb.append("-" + y);
  else if(b_abs_one && b_sign > 0)
    sb.append("+" + y);
  else if(b_sign < 0)
    sb.append(integer2string(b) + y);
  else if(b_sign > 0)
    sb.append("+" + integer2string(b) + y);
...
bool dyadic_inequality::operator==(const std::shared_ptr<inequality> i1) const
{
  if(cast_to_dyadic(i1))
  {
    std::shared_ptr<dyadic_inequality> d1 = cast_to_dyadic(i1);
    return (
      d1->x == this->x && d1->y == this->y && d1->a == this->a &&
      d1->b == this->b && d1->c == this->c);
  }
  else
  {
    return false;
